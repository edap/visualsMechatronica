...
vec2 map(vec3 pos){
    ...
    vec2 s3Mat = vec2(s3, 0.1);
    vec2 s4Mat = vec2(s4, 0.2);
    //vec2 s5Mat = vec2(s5, 0.2);
    vec2 s6Mat = vec2(s6, 0.3);

    return sminsMat(s4Mat, sminsMat(s3Mat, s6Mat));
}
