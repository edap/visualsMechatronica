...
float map(vec3 pos){
    pos.y += 1.;
    pos.xy = rotate(pos.xy, PI/2.0);
    pos.xz = rotate(pos.xz, cos(iGlobalTime*.2)*.7);

    float sdfOpRadius = 1.274;
    float sdrSmallR = 0.3;
    float nColums = 8. + (max(0., beat) * 3.);
    float blob = fBlob(pos);
    float torus = fTorus(pos, sdrSmallR, 0.79);
    return fOpDifferenceColumns(blob,torus,sdfOpRadius, nColums);
}
